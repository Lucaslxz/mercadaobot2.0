...
class PanelSystem {
  ...
  mapInteraction(interactionId, panelId) {
    ...
    logger.debug(`Interação ${interactionId} mapeada para painel ${panelId}`);
    return this;
  }
  ...
  mapInteractions(interactionIds, panelId) {
    if (!this.panels.has(panelId)) {
      throw new Error(`Painel ${panelId} não encontrado para mapear interações`);
    }

    for (const id of interactionIds) {
      this.interactions.set(id, panelId);
      logger.debug(`Interação ${id} mapeada para painel ${panelId}`);
    }
    return this;
  }
  ...
  async handleInteraction(interaction) {
    ...
    try {
      ...
      if (this.interactions.has(customId)) {
        const panelId = this.interactions.get(customId);
      ...
      if (!handled) {
        for (const [interactionId, panelId] of this.interactions.entries()) {
          if (customId.startsWith(interactionId)) {
            const panel = this.panels.get(panelId);
      ...
      // Verificar se o customId contém um caractere de separação '_'
      if (!handled && customId.includes('_')) {
        const prefix = customId.split('_')[0] + '_';

        if (this.interactions.has(prefix)) {
          const panelId = this.interactions.get(prefix);
          const panel = this.panels.get(panelId);
      ...
      if (!handled && (interaction.isButton() || interaction.isStringSelectMenu())) {
        logger.warn(`Interação não manipulada: ${interaction.customId}`);

        if (!interaction.replied && !interaction.deferred) {
          await interaction.reply({
            content: 'Este botão ou menu não está mais disponível.',
            ephemeral: true
          });
        }
      }
      ...
      logger.error(`Erro ao processar interação ${customId}:`, error);
